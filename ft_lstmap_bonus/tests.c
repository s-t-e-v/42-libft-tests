...
#include "../../libft.h"
#define FT "ft_lstmap"
#define FUNC(a, b, c) ft_lstmap(a, b, c)
...
static void	test1() {
	...
	mapped_lst = FUNC(lst, &times10, &del_int);
	printf("Mapped list:\n");
	if (mapped_lst)
		print_linked_list_int(mapped_lst);

	test_assert(check_content_int(mapped_lst, content_expected, size), test);

	ft_lstclear(&lst, &del_int);
	ft_lstclear(&mapped_lst, &del_int);
	printf("Done.\n");
